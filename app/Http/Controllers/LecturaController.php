...
class LecturaController extends Controller
{
    ...
    public function setCurrentReadings(Request $request): \Illuminate\Http\JsonResponse
    {
        $user = Auth::user();
        $validator = null;
        $error = false;
        ...
        $prevReading =  Lecturas::select('fechaMedicion')->orderBy('fechaMedicion', 'DESC')->first();
        ...
        $limitDate = now();
        if ($date_current->year === $limitDate->year && $date_current->month <= $limitDate->month && $user)
        {
            foreach ($request->json() as $clave => $valor)
            {
                ...
                if(HistorialCancelacion::verifyPaymentPending($data_lectura->medidor_id) % (int) $config_cancelations->cantidadMesesParaMulta === 0) {
    ...
    public function updateReading(Request $request, $id)
    {
        ...
        if ($user->tipoUsuario_id === 1)
        {
            ...
            if($readingPrevious && $request->input('lastReading') === $readingPrevious->medida)
            {
                $historyCancellation = HistorialCancelacion::pendingValid($id);
                if ($historyCancellation)
                {
                    $readingLastPrevious = Lecturas::previousReading($readingPrevious->medidor_id, $id);
                    if ($readingLastPrevious)
                    {
                        $recalculateDifference = (int) $request->input('currentReading') - $readingLastPrevious->medida;//$request->input('currentReading') - ($readingPrevious->medida - $historyCancellation->diferenciaMedida);
                    ...
                    {
                        $historyCancellation->diferenciaMedida = 0;
                        $historyCancellation->subTotal = (int) $request->input('currentReading') * $historyCancellation->precioUnidad;
                        $historyCancellation->save();
                        $readingPrevious->medida = $request->input('currentReading');
                        $readingPrevious->save();
                    }
    ...
    public function monthsReadings(): \Illuminate\Http\JsonResponse
    {
    ...
    public function monthsUntilNow(): \Illuminate\Http\JsonResponse
    {
        Carbon::setLocale('es');
        $firstMonthReading = Carbon::parse(Lecturas::orderBy('fechaMedicion')->select('fechaMedicion')->first()->fechaMedicion)->startOfMonth();
        $currentMonth = now()->startOfMonth();
        $listMonths = array();
        do {
            $listMonths [] = [
                "parserDate" => strtoupper("{$firstMonthReading->monthName} de {$firstMonthReading->year}"),
                "date" => $firstMonthReading->toDateTimeString()
            ];
            $firstMonthReading->setMonth($firstMonthReading->month + 1);
        } while($firstMonthReading <= $currentMonth);
        return response()->json([
            'success' => true,
            'data' => array_reverse($listMonths)
        ], 200);
    }
